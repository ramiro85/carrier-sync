...
class GoogleApiController:
    ...
    def authorize(self, username):
        raise NotImplementedError("GoogleApiController.authorize must be implemented by a subclass")
    ...
    def send_message(self, tms_message: TmsMessage):
        ...
        if tms_message.thread_id:
            last_msg = self.search_messages_by_thread_id(tms_message.username, tms_message.thread_id, 0)
    ...
    def search_messages_by_thread_id(self, username, thread_id, last=-1):
        ...
        return messages[last]  # return the last message in thread(the one with the attachment)
    ...
    def save_attachment(self, username, thread_id, filename, save_dir):
        ...
        for part in parts:
            if part['filename'] and 'attachmentId' in part['body']:
                ...
                if multiple:
                    file_path = os.path.join(save_path_temp if multiple else save_path, part['filename'])
        ...
        return ResponseModel(success=True, message=f"Saved attachment!", data={})

    def search_thread(self, username, query_subject, after=None, before=None, from_email=None):
        success, message, result, send_message = False, "", {}, None
        threads = self.find_thread(username=username, after=after, from_email=from_email)
        if threads:
    ...
    def find_messages(self, username, query_subject, after=None, before=None, from_email=None):
        success, message, result, send_message = False, "", [], None
        threads = self.find_thread(username=username, after=after, from_email=from_email)
