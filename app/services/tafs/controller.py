...
def select_best_debtor(debtor_list):
    """Pick the most actionable debtor entry, preferring non-denied buy status."""
    best_debtor = None
...
class TafsController:
    ...
    def _update_asp_state(self, html_content):
        ...
        if vsg:
            self.view_state_gen = vsg.get("value", "")
        if ev:
    ...
    def is_authenticated(self):
        ...
        try:
            response = self.session.get(
                self.url_debtor, headers=self.headers, timeout=10
            )
    ...
    def get_view(self, url):
        """Open a TAFS page and refresh ASP.NET state tokens for subsequent requests."""
        ...
        if "login.aspx" in response.url or response.status_code != 200:
            ...
            response = self.session.get(
                url, verify=certifi.where(), allow_redirects=True
            )

        self._update_asp_state(response.content)

    def search_broker(self, query: str):
        """Search broker data and enrich the result with SAFER and debtor details."""
        if query.isnumeric():
            filter_text = query
        else:
            filter_text = f"%{query}%"
        ...
        if no_records is not None:
            broker_info["tafs_debtor"] = TafDebtor.model_validate(
                {"debtor_msg": "No records found in TAFS for this MC number"}
            )
            return broker_info
        else:
            if _clientKeyValues:
                word_list = re.split(
                    r"_clientKeyValues\"\:|\,\"_controlToFocus",
                    _clientKeyValues[0],
                    flags=re.IGNORECASE,
                )
    ...
    def load_debtor(self, account_id):
        """Load debtor details for a TAFS account."""
        req_payload = self.payload.copy()
        req_payload["ctl00$db_script_manager"] = (
            "ctl00$db_script_manager|ctl00$cnt_main$btn_load_debtor"
        )
        req_payload["__VIEWSTATEGENERATOR"] = self.view_state_gen
        req_payload["__VIEWSTATE"] = (self.view_state,)
        req_payload["__EVENTVALIDATION"] = (self.event_validation,)
        ...
        if response:
            ...
            if soup2:
                debtor_buy_status = soup2.find(
                    attrs={"id": "cnt_main_debtor_status_lbl_buy_status"}
                )
                debtor_rating = soup2.find(
                    attrs={"id": "cnt_main_debtor_status_lbl_rating"}
                )
                debtor_credit_limit = soup2.find(
                    attrs={"id": "cnt_main_debtor_status_lbl_credit_limit"}
                )
                ...
                debtor_msg = soup2.find(
                    attrs={"id": "cnt_main_debtor_status_lbl_debtor_msg"}
                )

                modal_body = soup2.find(attrs={"class": "modal-body"})
                tafs_html = ""
                if modal_body:
                    children = modal_body.find("div", recursive=False).findChildren(
                        "div", recursive=False
                    )
